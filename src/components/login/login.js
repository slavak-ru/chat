...
export default class Login {
  /**
	...
	*/
  constructor({element, usersUrl, networkService, form, tooltip, users, modal}) {
    this.logins = element;
    this.usersUrl = usersUrl;
    this.networkService = networkService;
    this.form = form;
    this.tooltip = tooltip;
    this.users = new users({usersUrl: this.usersUrl, networkService: this.networkService});
    this.modal = modal;
    ...
    this.forgotErrorText = 'Пользователя с таким логином и мейлом не существует';
    this.tabsList = {};
  ...
  _initEvents() {
    this.logins.addEventListener('click', (e)=>{
      let elem = e.target;
      if(!this.tabsList[elem.value] && !elem.classList.contains('icon-password')) return;
  ...
  /**
		...
		* @description Inner method - creating loggin form.
