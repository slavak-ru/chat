...
/** 
 * @class Message
 * @description Class Message add messagees in Chat (DOM-element). 
 * */
export default class Message {
  ...
  constructor({element, messages = [], currentUser}) {
    this.chat = element;
    this.messages = Array.isArray(messages)? messages : Object.values(messages);
  ...
  render() { 
    ...
    if(!this.messages[this.count].message.length || 
  ...
  _createChatElements() {
    ...
    for (this.count; this.count < this.messages.length; this.count +=1) {
      ...
      div.className = (this.messages[this.count].user !== this.currentUser)? 
                      'message-wrapper':
